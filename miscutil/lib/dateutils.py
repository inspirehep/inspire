...
def convert_datetext_to_dategui(datetext, ln=cdslang):
    ...
    try:
        ...
        if datestruct == datestruct_default:
            raise ValueError
        return(convert_datestruct_to_dategui(datestruct, ln))
    except:
...
def convert_datetext_to_datestruct(datetext):
    ...
    try:
        ...
        no_day_format= "%Y-%m-00 %H:%M:%S"
        try:
            struct=strptime(datetext, no_day_format)
            return((struct[0],struct[1],0,0,0,0,0,0,0))
        except:
             no_month_format= "%Y-00-00 %H:%M:%S"
             try:
                 struct= strptime(datetext, no_month_format)
                 return((struct[0],0,0,0,0,0,0,0,0))
             except:
