...
def parse_ref(markfield):
    ...
    Return a dict with text line, curator-tag, $$0, $$r, $$s mark tags,
    DOI and text fields hmxt
    """

    reference_dict = {'subfield_0': '', 'doi': '', 'repno': '',
                      'subfield_pbn': '', 'subfields_text': '',
                      'position_pbn': -1, 'mark_line': '', 'curator': ''}
...
def confirm_by_authors(text, candidate):
    ...
    Return reason - result of confirmation analysis
    If the cited record has more than 8 authors most likely only the first is
    given in the reference (et.al.)
    """
